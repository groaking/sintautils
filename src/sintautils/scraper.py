...
class AV(SintaScraper):
    ...
    def _scrape_wos(self, author_id: str, out_format: str = 'json', fields: list = ['*']):
    ...
    def get_wos(self, author_id: list = [], out_format: str = 'csv', fields: list = ['*']):
        ...
        if type(author_id) is str:
            a = self._scrape_wos(author_id=str(author_id), out_format=out_format, fields=fields)

        elif type(author_id) is list:
            a = []
            for l in author_id:
                self.print(f'Scraping for author ID: {l}...', 2)
                a.extend(self._scrape_wos(author_id=l, out_format=out_format, fields=fields))
